...
#adding the pfts accordingt to Tom's crtieria in his 2018 New Phyt paper
...
#adding the loss back to the recruitment rates
...
rec_adjust_data
#pick up here and double check how this worked
#early 00-05


rec_adjusted <- matrix(nrow = 3, ncol = 3)
for(i in 2:3){
  for(j in 2:3){
    rec_adjusted[i,j] = rec_adjust(R = bci_rec_bench[i,j], M = mort_rates[i,j])
  }
}
...
#early 00-05
rec_adjust(R = bci_rec_bench[1,2], M = mort_rates[1,2])
...
ruger_params <- read.csv("ruger_params_clean.csv")
str(ruger_params)
names(ruger_params)[1] <- "Latin"
ruger_clean <- merge(ruger_params[c(1,5,11)], canopy_sp[c(1,12,14)], by = "Latin")
...
a_param <- log10(bci_rec_bench[1] / (10000/25)) %>% .$R_05to10
