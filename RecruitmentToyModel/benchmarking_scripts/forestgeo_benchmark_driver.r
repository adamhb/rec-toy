...
ctfsrpackfile <- "C:/Users/ahanb/OneDrive/Documents/tools_benchmarking_evaluation/forestgeo_2_dembmarks/CTFSRPackage.rdata"
site.name <- "bci"

# Full path to the CTFS plot species data (plot species table needs to be in working directory)
sp_table_name<-list.files(path="C:/Users/ahanb/OneDrive/Documents/tools_benchmarking_evaluation/forestgeo_2_dembmarks/", pattern = paste(site.name,".spptable.rdata",sep = ""),full.names = TRUE)
...
source("fgcb_utilities.r")   # Error logging (currently)
source("fgcb_plot_utils.r")  # Specialized Plots
...
xmlsettings <- read_xml("C:/Users/ahanb/OneDrive/Documents/rec_submodel/rec_submodel_scripts/RecruitmentToyModel/benchmarking_scripts/ctfs_at_settings.xml")
...
files <- list.files(path = rtablepref, pattern = paste(site.name, ".", ctfs_table_type, 
                                                       sep = ""), full.names = TRUE)
