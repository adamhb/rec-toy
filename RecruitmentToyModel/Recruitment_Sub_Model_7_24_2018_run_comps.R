...
driver_data_path <- "C:/Users/ahanb/OneDrive/Documents/Recruitment_Toy_Model_non_Git/Driver_Data/"
#setwd(driver_data_path)

bci_precip_new <- read.csv(paste0(driver_data_path,"bci_rain_new.csv")) #read in the input precipitation data
bci_precip_new$date <- strptime(as.character(bci_precip_new$date), format = "%m/%d/%Y") %>% as.Date(.)


#add working directory to the location of the input data
...
#importing the FATES output data to use as driver data for the toy model
setwd(paste0(driver_data_path,"FATES_output_May_2018"))
...
age.n <- c(rep("mature", 4), rep("young", 4))
light.n <- rep(c("3pct", "3pct", "15pct", "15 pct"),2)
moisture.n <- rep(c("amb","dry"),4)

age <-  c(rep(500, 4), rep(30, 4))
light.xxx <-  rep(c(0.03, 0.03, 0.15, 0.15),2)

run_name <- paste(age.n[z], light.n[z], moisture.n[z])

ifelse(moisture.n[z] == "dry", SMP <- SMP_dry, SMP <- SMP)

#parameters
#scenario
percent_light <- light.xxx[z]
dbh.x <- age[z] #dbh in mm
...
#seedling mort-H20
background_seedling_mort <- 0.36 #this has been doubled
...
run_comparisons[[run_name]] <- full_output %>% mutate(year = substring(text = as.character(date), first = 1, last = 4)) %>% group_by(pft,dpft,year) %>% summarise(N_rec = sum(R)) %>% summarise(avg_annual_n_rec = mean(N_rec)) %>% mutate(age = age.n[z], light = light.n[z], moisture = moisture.n[z])
...
dir.create(path = paste0("C:/Users/ahanb/OneDrive/Documents/Recruitment_Toy_Model_non_Git/Output_for_Lara/",run_name), showWarnings = T)
...
write.csv(paramsOFrun, file = paste0("C:/Users/ahanb/OneDrive/Documents/Recruitment_Toy_Model_non_Git/Output_for_Lara/",run_name,"/params.csv"))


setwd("C:/Users/ahanb/OneDrive/Documents/Recruitment_Toy_Model_non_Git/")
