...
frac_rec <- function(light = 5e6, t_window.x = c(8,6), PFT = "early", seedlings_C = 750000, N_smp = 500, p1e.x = 0.7618, p1l.x = 0.2211, p2e.x = 0.0099, p2l.x = 0.0145, g_param = c(1.00, 1.00)){
 ...
 # ifelse(test = PFT == "early" & pct_light > 1, yes =  t_window <- t_window.x / (log(pct_light)), no = t_window <- t_window.x)
  ...
  p1 <- rnorm(mean = p1, sd = p1_sd*1.2, n = N_smp)
  
  p2 <- rnorm(mean = p2, sd = p2_sd*1.2, n = N_smp)
