...
#CREATING THE TIME SERIES
  ...
  for(PFT in c("early", "late")){
    ...
    cmc_ind[1] <- cmc_ind_0
    co_dbh_ind[1] <- C2dbh(cmc_ind[1])
    co_LAI_ind[1] <- dbh2LAI(dbh = co_dbh_ind[1])
    N_co[1] <- total_area * LAI_cohort / co_LAI_ind[1]
    c4repro[1] <- 0
    PFT.x[1] <- PFT
    
    for(i in 1:((years*17520)-1)){
 
   #the number of 30 min timesteps in the model run
      ...
      N_co[i+1] <- total_area * LAI_cohort / co_LAI_ind[i+1]
      
      c4repro[i+1] <- (N_co[i] * co_LAI_ind[i] * rad[i] * epsilon * frac_NPP2GPP * efrac(N = N_co[i], co_dbh_ind = co_dbh_ind[i]))
