...
source(dir("C:/Users/ahanb/OneDrive/Documents/rec_submodel/rec_submodel_scripts/RecruitmentToyModel/funcs", pattern = ".R"))
...
light_output <- c()
for(Q in c(0.03, 0.06, 0.09, 0.12, 0.15)){
  
run_name <- as.character(Q)
...
light_output[[run_name]] <- full_output %>% mutate(year = substring(text = as.character(date), first = 1, last = 4)) %>% group_by(pft,year) %>% summarise(N_rec = sum(R)) %>% summarise(avg_annual_n_rec = mean(N_rec)) %>% mutate(light = run_name)
